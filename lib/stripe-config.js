...
function validateStripeKeys() {
  ...
  if (!publishableKey) {
    console.warn(`⚠️ [STRIPE CONFIG] Missing NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY (optional for backend-only operations)`);
  }
  ...
  if (IS_TEST_MODE) {
    ...
    if (publishableKey && !publishableKey.startsWith('pk_test_')) {
      throw new Error(
        `[STRIPE CONFIG] STRIPE_MODE=test but NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY is not a test key (must start with pk_test_)`
      );
    }
  }
  ...
  if (IS_LIVE_MODE) {
    ...
    if (publishableKey && !publishableKey.startsWith('pk_live_')) {
      throw new Error(
        `[STRIPE CONFIG] STRIPE_MODE=live but NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY is not a live key (must start with pk_live_)`
      );
    }
  }
