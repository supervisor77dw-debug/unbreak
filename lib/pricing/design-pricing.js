...
export function priceDesign(payload, options = {}) {
  ...
  // VALIDATION RULE: Every design MUST resolve to exactly ONE base product SKU
  if (!payload.baseComponents || payload.baseComponents.length === 0) {
    errors.push('HARD_FAIL: No base components in payload');
  } else if (payload.baseComponents.length > 1) {
    errors.push('HARD_FAIL: Multiple base components not allowed (must be exactly 1)');
  } else {
  ...
  // VALIDATION RULE: customization.enabled = true requires valid fee configured
  if (payload.customization?.enabled) {
    if (!payload.customization?.feeKey) {
      errors.push('HARD_FAIL: Customization enabled but no feeKey provided');
    } else {
      const fee = getCustomizationFee(payload.customization.feeKey);
      
      if (!fee) {
        errors.push(`HARD_FAIL: Unknown customization fee: ${payload.customization.feeKey}`);
      } else {
      customizationFee = fee.amount;
        ...
        breakdownLines.push({
          ...
          title: fee.label || fee.description,
  ...
  if (payload.premiumAddons && payload.premiumAddons.length > 0) {
    for (const addon of payload.premiumAddons) {
      ...
      if (!addonInfo) {
        errors.push(`HARD_FAIL: Unknown premium addon pricingKey: ${pricingKey}`);
        continue;
      }
      ...
      breakdownLines.push({
        type: 'addon',
        pricingKey,
        label: label || addonInfo.label || addonInfo.description,
        qty,
        unitPrice,
        lineTotal,
