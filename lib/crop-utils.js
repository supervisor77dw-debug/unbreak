...
export function computeCropRectOriginalPx(origW, origH, targetAspect = 0.8, scale = 1.0, x = 0, y = 0) {
  ...
  // This is the rect size at scale=1.0 (no zoom)
  ...
  // STEP 2: Calculate effectiveScale
  // CRITICAL: effectiveScale = baseScale / userScale
  // - baseScale: minimal scale to cover 4:5 frame (CSS cover-fit)
  // - userScale: zoom factor (>1.0 = zoom in)
  // - effectiveScale: DECREASES with zoom (inverse relationship!)
  //
  // This makes:
  // - cropRect SMALLER when zooming in (correct!)
  // - offsets WEAKER when zooming in (correct!)
  const baseScale = Math.max(baseWidth / origW, baseHeight / origH);
  const effectiveScale = baseScale / safeCrop.scale;
  
  // STEP 3: Calculate crop rect dimensions
  // cropRect shrinks when zooming: scale 1.0→1.8 makes rect size / 1.8
  const cropW = Math.round(origW * effectiveScale);
  const cropH = Math.round(origH * effectiveScale);
  
  // STEP 4: Map offsets from reference space (900×1125) to original pixels
  // Offsets scale with effectiveScale, so they get WEAKER when zooming
  // x=-60 at scale=1.0 → larger offset
  // x=-60 at scale=1.8 → smaller offset (rect is smaller, offset proportional)
  const offsetXOrig = safeCrop.x * effectiveScale;
  const offsetYOrig = safeCrop.y * effectiveScale;
  
  // STEP 5: Calculate extract position in original pixels
  // The crop rect is centered, then offset
  const centerX = origW / 2;
  const centerY = origH / 2;
  
  const left = centerX - cropW / 2 + offsetXOrig;
  const top = centerY - cropH / 2 + offsetYOrig;
  
  // STEP 6: Round to integers (ONCE, consistently)
  let rectLeft = Math.round(left);
  let rectTop = Math.round(top);
  let rectWidth = Math.round(cropW);
  let rectHeight = Math.round(cropH);
  
  // STEP 7: Clamp to original bounds (ONCE, consistently)
  const clampedLeft = Math.max(0, Math.min(rectLeft, origW - rectWidth));
  const clampedTop = Math.max(0, Math.min(rectTop, origH - rectHeight));
  const clampedWidth = Math.max(1, Math.min(rectWidth, origW - clampedLeft));
  const clampedHeight = Math.max(1, Math.min(rectHeight, origH - clampedTop));
  
  const wasClamped = clampedLeft !== rectLeft || clampedTop !== rectTop || 
                     clampedWidth !== rectWidth || clampedHeight !== rectHeight;
  ...
  return {
    ...
    debug: {
      ...
      cropSize: `${cropW.toFixed(1)}×${cropH.toFixed(1)}`,
      ...
      formula: `effectiveScale = baseScale(${baseScale.toFixed(4)}) / userScale(${safeCrop.scale}) = ${effectiveScale.toFixed(4)}`,
    }
  };
}
