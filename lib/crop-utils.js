...
export function computeCropRectOriginalPx(origW, origH, targetAspect = 0.8, scale = 1.0, x = 0, y = 0) {
  ...
  // CRITICAL: scale > 1.0 means ZOOM IN → cropRect gets SMALLER
  // scale = 1.0 → full base dimensions (normal cover-fit)
  // scale = 2.0 → half base dimensions (2x zoom)
  const zoomedWidth = baseWidth / safeCrop.scale;
  const zoomedHeight = baseHeight / safeCrop.scale;
