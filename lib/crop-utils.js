...
export function computeExtractRect({ origW, origH, targetW, targetH, scale = 1.0, x = 0, y = 0 }) {
  ...
  // 4. Scale offsets from base coordinate space to resized image space
  // CRITICAL: Offsets are defined in base frame (targetW x targetH)
  // They must be scaled by effectiveScale to match resized image
  const scaledOffsetX = safeCrop.x * effectiveScale;
  const scaledOffsetY = safeCrop.y * effectiveScale;
  
  // 5. Calculate extract position (centered + scaled offsets)
  ...
  const left = Math.round(centerX - targetW / 2 + scaledOffsetX);
  const top = Math.round(centerY - targetH / 2 + scaledOffsetY);
  ...
  return {
    ...
    debug: {
      ...
      offsetBase: `(${safeCrop.x}, ${safeCrop.y})`,
      offsetScaled: `(${scaledOffsetX.toFixed(2)}, ${scaledOffsetY.toFixed(2)})`,
      extractRect: `left=${clampedLeft}, top=${clampedTop}, w=${targetW}, h=${targetH}`,
      wasClamped: clampedLeft !== left || clampedTop !== top,
      yAppliedBeforeZoom: false, // Applied in base space, then scaled
      ySign: '+', // Positive Y moves down (standard image coordinates)
      yScaleFactor: 'effectiveScale', // Offsets scaled by effectiveScale
    }
  };
}
