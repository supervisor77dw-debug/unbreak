...
async function getPricingConfig(variant) {
  ...
  if (error) {
    ...
    const { DEFAULT_COLOR_PRICES, ADAPTER_DEFAULT_COLOR_PRICES } = await import('../constants/colors.js');
    return {
      variant,
      base_price_cents: 4990,
      color_prices: {
        base: { ...DEFAULT_COLOR_PRICES },         // 7 Farben (ohne grey)
        arm: { ...DEFAULT_COLOR_PRICES },          // 7 Farben (ohne grey)
        module: { ...ADAPTER_DEFAULT_COLOR_PRICES }, // 5 Farben (mit grey)
        pattern: { ...DEFAULT_COLOR_PRICES },      // 7 Farben (ohne grey)
      },
  ...
  // Ensure all required colors are present (part-specific)
  const { DEFAULT_COLOR_PRICES, ADAPTER_DEFAULT_COLOR_PRICES } = await import('../constants/colors.js');
  ['base', 'arm', 'pattern'].forEach(part => {
    if (!data.color_prices[part]) {
      data.color_prices[part] = { ...DEFAULT_COLOR_PRICES }; // 7 Farben
    } else {
  ...
  // Module/Adapter: Separate handling (5 colors with grey)
  if (!data.color_prices.module) {
    data.color_prices.module = { ...ADAPTER_DEFAULT_COLOR_PRICES }; // 5 Farben
  } else {
    data.color_prices.module = {
      ...ADAPTER_DEFAULT_COLOR_PRICES,
      ...data.color_prices.module,
    };
  }
