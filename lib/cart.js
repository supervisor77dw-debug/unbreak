...
class Cart {
  ...
  load() {
    ...
    try {
      ...
      if (validItems.length !== items.length) {
        if (DEBUG) console.log('[CART][LOAD][MIGRATION] ⚠️ Cleaned cart on load:', {
          before: items.length,
        ...
        if (DEBUG) console.log('[CART][LOAD] ✅ All items valid, no migration needed. Items:', items.length);
      }
  ...
  addItem(product) {
    ...
    if (existingIndex >= 0) {
      ...
      // CRITICAL FIX: Prefer price_cents (configurator uses this)
      // Priority: price_cents > price > unit_amount > base_price_cents
      const price = product.price_cents || product.price || product.unit_amount || product.base_price_cents;
      
      // For configurator items (with config), price will be calculated later
      // For standard products, price is required
      if (!product.config && (!Number.isFinite(Number(price)) || Number(price) <= 0)) {
        console.error('[CART][ADD_ITEM] Invalid price for standard product:', product);
        return false;
      }

      // Debug logging for configurator items
      if (product.configured && DEBUG) {
        console.log('[CART][ADD_ITEM][CONFIG] Price sources:', {
      ...
      const newItem = {
        ...
        price: Number(price) || 0, // Allow 0 for configurator items (server calculates)
        quantity: 1,
        ...
        lang: product.lang || null, // CRITICAL: Preserve language for i18n
      };
      
      if (DEBUG) console.log('[CART][ADD_ITEM] Adding to cart:', {
        product_id: newItem.product_id,
        sku: newItem.sku,
        configured: newItem.configured,
        has_config: !!newItem.config,
        config_keys: newItem.config ? Object.keys(newItem.config) : null
      });
  ...
  getCheckoutPayload() {
    return this.items.map(item => ({
      ...
      sku: item.sku,
      // Include price for pricing calculation (standard products need this)
