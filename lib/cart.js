...
class Cart {
    ...
    try {
      ...
      const validItems = items.filter(item => {
        ...
        if (isConfiguratorItem && !item.config) {
          console.warn('[CART][LOAD][MIGRATION] ❌ Removing old configurator item without config:', {
      ...
      if (validItems.length !== items.length) {
        console.log('[CART][LOAD][MIGRATION] ⚠️ Cleaned cart on load:', {
          before: items.length,
        ...
        console.log('[CART][LOAD] ✅ All items valid, no migration needed. Items:', items.length);
      }
  ...
  save() {
    ...
    try {
      ...
      console.log('[CART][SAVE] Saving to localStorage:', {
      ...
      console.log('[CART][SAVE] ✅ Saved successfully. Verified count:', parsed.length);
  ...
  addItem(product) {
    ...
    if (existingIndex >= 0) {
      ...
      const newItem = {
        ...
        price: Number(price) || 0, // Allow 0 for configurator items
        quantity: 1,
      ...
      console.log('[CART][ADD_ITEM] Adding to cart:', {
    ...
    console.log('[CART][ADD_ITEM] Cart saved. Items count:', this.items.length);
