 ...
 * 
 * CRITICAL: targetAspect ist IMMER 4:5 (0.8) - NICHT targetW/targetH!
 * Die x/y Offsets sind in 900×1125 "Reference Space" definiert.
 * 
 ...
 * @param {number} x - User crop X offset in REFERENCE pixels (900×1125 space)
 * @param {number} y - User crop Y offset in REFERENCE pixels (900×1125 space)
...
export function computeCropRectOriginalPx(origW, origH, targetAspect = 0.8, scale = 1.0, x = 0, y = 0) {
  ...
  // STEP 1: Calculate base dimensions for 4:5 aspect in original space
  // We need to determine the "reference frame" size that would cover the original
  // with 4:5 aspect ratio
  ...
  // effectiveScale scales the base dimensions
  const effectiveScale = safeCrop.scale;
  const zoomedWidth = baseWidth * effectiveScale;
  const zoomedHeight = baseHeight * effectiveScale;
  
  // STEP 3: Map offsets from reference space (900×1125) to original space
  // Reference space is 900×1125, base space in original is baseWidth×baseHeight
  const REFERENCE_W = 900;
  const REFERENCE_H = 1125;
  
  // Scale factor to convert reference pixels to original pixels
  const refToOrigScale = baseWidth / REFERENCE_W;
  
  const offsetXOrig = safeCrop.x * refToOrigScale;
  const offsetYOrig = safeCrop.y * refToOrigScale;
  
  // STEP 4: Calculate extract position in original pixels
  // The zoomed area is centered, then offset
  const centerX = origW / 2;
  const centerY = origH / 2;
  
  const left = centerX - zoomedWidth / 2 + offsetXOrig;
  const top = centerY - zoomedHeight / 2 + offsetYOrig;
  
  // STEP 5: Round to integers (ONCE, consistently)
  let rectLeft = Math.round(left);
  let rectTop = Math.round(top);
  let rectWidth = Math.round(zoomedWidth);
  let rectHeight = Math.round(zoomedHeight);
  
  // STEP 6: Clamp to original bounds (ONCE, consistently)
  rectLeft = Math.max(0, Math.min(rectLeft, origW - 1));
  rectTop = Math.max(0, Math.min(rectTop, origH - 1));
  rectWidth = Math.max(1, Math.min(rectWidth, origW - rectLeft));
  rectHeight = Math.max(1, Math.min(rectHeight, origH - rectTop));
  ...
  return {
    ...
    debug: {
      ...
      baseSize: `${baseWidth.toFixed(1)}×${baseHeight.toFixed(1)}`,
      userScale: safeCrop.scale.toFixed(2),
      zoomedSize: `${zoomedWidth.toFixed(1)}×${zoomedHeight.toFixed(1)}`,
      offsetRef: `(${safeCrop.x}, ${safeCrop.y})`,
      offsetOrig: `(${offsetXOrig.toFixed(1)}, ${offsetYOrig.toFixed(1)})`,
      cropRect: `[${rectLeft}, ${rectTop}, ${rectWidth}, ${rectHeight}]`,
      // Simple hash for debugging (no crypto needed in browser)
      hash: `${rectLeft}_${rectTop}_${rectWidth}_${rectHeight}`,
    }
  };
}

/**
 * SINGLE SOURCE OF TRUTH: Berechnet Transform für 4:5 Crop (CSS Preview)
 * 
 * MUST produce visually identical result to computeCropRectOriginalPx!
 * 
 * Approach: Simulate extract().resize() in CSS
 * 1. Compute the cropRect in original pixels (using computeCropRectOriginalPx)
 * 2. Display that extracted portion scaled to fill the frame
 * 
 * @param {Object} params
 * @param {number} params.imgW - naturalWidth des Bildes  
 * @param {number} params.imgH - naturalHeight des Bildes
 * @param {number} params.frameW - Zielrahmen Breite (z.B. 400px in browser)
 * @param {number} params.frameH - Zielrahmen Höhe (z.B. 500px für 4:5)
 * @param {number} params.scale - User Crop Scale (1.0 - 2.5)
 * @param {number} params.x - User Crop X in 900×1125 reference pixels
 * @param {number} params.y - User Crop Y in 900×1125 reference pixels
...
export function computeCoverTransform({ imgW, imgH, frameW, frameH, scale = 1.0, x = 0, y = 0 }) {
  ...
  return { 
    transform, 
    baseScale: fillScale, // For backwards compatibility
    effectiveScale: fillScale,
    cropRect, // Expose for debugging
    // Debug-Info
    debug: {
      imgSize: `${imgW}x${imgH}`,
