...
function getAllowedOrigins() {
  const origins = [
    ...
    'https://unbreak-3-d-konfigurator.vercel.app', // EXPLICIT: Production configurator
    'https://www.unbreak-one.com', // EXPLICIT: Production shop (www)
    'https://unbreak-one.com', // EXPLICIT: Production shop (non-www)
  ];
  
  // Add production domains from ENV if available
  if (process.env.NEXT_PUBLIC_CONFIGURATOR_DOMAIN) {
    origins.push(process.env.NEXT_PUBLIC_CONFIGURATOR_DOMAIN);
  }
  ...
  if (process.env.NEXT_PUBLIC_CONFIGURATOR_CUSTOM_DOMAIN) {
    origins.push(process.env.NEXT_PUBLIC_CONFIGURATOR_CUSTOM_DOMAIN);
  }
