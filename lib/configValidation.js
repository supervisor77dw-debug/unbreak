 ...
 * 
 * These 7 colors match the 3D configurator's COLOR_PALETTE exactly:
 * - mint: #a2d9ce
 * - green: #145a32
 * - purple: #4a235a
 * - iceBlue: #5499c7
...
export const ADAPTER_ALLOWED_COLOR_IDS = [
  ...
  'darkBlue',  // Blau
  'green',     // Grün
  'mint'       // Grau (re-labeled for adapter context)
];
...
export function validateConfiguratorConfig(config, traceId = 'unknown') {
  ...
  for (const part of requiredParts) {
    ...
    if (part === 'module' && colorId && !isAdapterColorAllowed(colorId)) {
      // Legacy/invalid color detected - fallback to black
