...
// Base/Arm/Pattern: 7 Farben (OHNE grey)
export const DEFAULT_COLOR_PRICES = CONFIGURATOR_COLORS.reduce((acc, color) => {
  acc[color.id] = 0;
  return acc;
}, {});

// Adapter/Module: NUR 5 Farben (MIT grey, OHNE mint/purple/dark_blue)
export const ADAPTER_DEFAULT_COLOR_PRICES = ADAPTER_ALLOWED_COLORS.reduce((acc, color) => {
  acc[color.id] = 0;
  return acc;
}, {});
