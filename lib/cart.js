  ...
  load() {
    ...
    try {
      ...
      const normalizedItems = validItems.map(item => {
        // Check if item needs normalization (has legacy price fields)
        const hasLegacyFields = !item.unit_price_cents || item.price || item.price_cents || item.base_price;
        
        if (hasLegacyFields) {
          needsMigration = true;
          if (DEBUG) console.log('[CART][LOAD][MIGRATION] Normalizing legacy item:', {
            sku: item.sku,
            has_unit_price_cents: !!item.unit_price_cents,
            has_price: !!item.price,
            has_price_cents: !!item.price_cents,
          });
        }
      ...
      // If we cleaned or normalized items, save immediately
      if (validItems.length !== items.length || needsMigration) {
        if (DEBUG) console.log('[CART][LOAD][MIGRATION] 🔄 Migrated cart:', {
          before: items.length,
          after: normalizedItems.length,
          removed: items.length - normalizedItems.length,
