...
export function resolveCartItemPrice(item, pricingConfig = null, productsIndex = null) {
  ...
  // =================================================================
  // P0 FIX: CONFIGURATOR ITEMS USE ADMIN PRICING
  // Client price is COMPLETELY IGNORED - Admin Pricing is Source of Truth
  // =================================================================
  if (item.configured || item.product_id === 'glass_configurator') {
    if (DEBUG) {
      console.log('[PRICE_RESOLVER][CONFIGURATOR][P0_FIX]', {
        sku: item.sku,
        source: 'admin_pricing_lookup',
        has_config: !!item.config,
        client_price_IGNORED: item.price_cents || item.price || 'none',
      });
    }

    // ❌ IGNORE client price fields completely
    // ✅ Lookup price from Admin Pricing (same as standard products)
    // Fall through to standard product logic below
  }
