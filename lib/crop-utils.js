...
export function isValidCropState(crop) {
  ...
  if (crop.cropVersion === 2) {
    return isFiniteNumber(crop.nx) && isFiniteNumber(crop.ny);
  }
...
export function clampCropState(crop, imageSize, containerSize) {
  ...
  // V2: Clamp normalized offsets (simpler, no pixel calculations needed)
  if (safeCrop.cropVersion === 2) {
    // Normalized offsets are relative to viewport, no clamping needed
    // (Server will clamp when converting to pixels)
    const result = { 
      scale, 
      nx: safeCrop.nx, 
      ny: safeCrop.ny,
      cropVersion: 2,
      x: 0,  // Keep for compatibility
      y: 0
    };
    
    if (!isValidCropState(result)) {
      console.error('[crop-utils] clampCropState V2: INVALID RESULT!', result);
      return { scale: 1.0, nx: 0, ny: 0, cropVersion: 2, x: 0, y: 0 };
    }
    
    return result;
  }
  
  // V1: Clamp pixel offsets (legacy)
  ...
  const result = { scale, x, y, cropVersion: 1 };
  if (!isValidCropState(result)) {
    console.error('[crop-utils] clampCropState V1: INVALID RESULT!', result);
    return { scale: 1.0, x: 0, y: 0, cropVersion: 1 };
  }
