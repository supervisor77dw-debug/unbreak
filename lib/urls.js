 ...
 * SSR-safe with multiple fallbacks
 * @returns {string} Base URL without trailing slash
 */
export function getSiteUrl() {
  // Server-side: use ENV with Vercel URL fallback
  if (typeof window === 'undefined') {
    // Explicit site URL (preferred)
    const explicit = process.env.NEXT_PUBLIC_SITE_URL || process.env.SITE_URL;
    if (explicit) return explicit.replace(/\/$/, '');
    
    // Vercel URL fallback (preview deployments)
 ...
 * 
 * CRITICAL: Always pass shop_origin + return_path for dynamic redirects
 * This ensures Preview → Preview, Production → Production
 * 
 * @param {string} lang - Language (de, en)
 * @param {string} [returnUrl] - Optional return URL (full URL or path)
...
export function buildConfiguratorUrl(lang = 'de', returnUrl = null) {
  ...
  // Determine shop origin (current domain)
  // CRITICAL: Use window.location.origin in browser for Preview support
  let shopOrigin;
  if (typeof window !== 'undefined') {
    shopOrigin = window.location.origin; // Preserves preview URLs
  } else {
    shopOrigin = getSiteUrl(); // Server-side fallback
  }
  
  // Determine return path (relative to shop origin)
  let returnPath = '/shop'; // Default
  if (returnUrl) {
    ...
    if (returnUrl.startsWith('http')) {
      try {
        ...
        returnPath = '/shop'; // Fallback on parse error
      }
    } else {
      // Already a path
  ...
  const params = new URLSearchParams({
    lang: lang || 'de',
    shop_origin: shopOrigin,      // ← NEW: Dynamic origin
    return_path: returnPath,       // ← NEW: Relative path
  });
