...
export function computeCropRectOriginalPx(origW, origH, targetAspect = 0.8, scale = 1.0, x = 0, y = 0) {
  ...
  // STEP 3: Offset mapping (reference preview → original pixels)
  // Reference space: 900×1125 (UI preview coordinate system)
  // baseScale: How much the base box was scaled to fit in reference space
  // effectiveScale: Combined scale including user zoom
  const baseScale = baseW / REFERENCE_W;
  const effectiveScale = baseScale / safeCrop.scale;
  
  const offsetXOrig = Math.round(safeCrop.x * effectiveScale);
  const offsetYOrig = Math.round(safeCrop.y * effectiveScale);
  ...
  // Center the crop rect, then apply offset
  const centerX = Math.round(origW / 2);
  const centerY = Math.round(origH / 2);
  
  const left = Math.round(centerX - cropW / 2 + offsetXOrig);
  const top = Math.round(centerY - cropH / 2 + offsetYOrig);
  ...
  return {
    ...
    debug: {
      ...
      offsetOrig: `(${offsetXOrig}, ${offsetYOrig})`,
      cropRect: `[${clampedLeft}, ${clampedTop}, ${clampedWidth}, ${clampedHeight}]`,
      wasClamped,
      hash: `${clampedLeft}_${clampedTop}_${clampedWidth}_${clampedHeight}`,
      formula: `cropW=${baseW}/${safeCrop.scale}=${cropW}, effectiveScale=${baseScale.toFixed(4)}/${safeCrop.scale}=${effectiveScale.toFixed(4)}`,
      invariants: {
