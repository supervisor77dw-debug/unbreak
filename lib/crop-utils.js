...
export function computeCropRectOriginalPx(origW, origH, targetAspect = 0.8, scale = 1.0, x = 0, y = 0) {
  ...
  // STEP 3: Map offsets from reference space (900×1125) to ZOOMED original space
  // CRITICAL: Offsets are relative to the ZOOMED rect, not base rect!
  // Reference space is 900×1125, zoomed space in original is zoomedWidth×zoomedHeight
  const REFERENCE_W = 900;
  const REFERENCE_H = 1125;
  
  // Scale factor to convert reference pixels to zoomed original pixels
  // This automatically accounts for zoom level!
  const refToZoomedScale = zoomedWidth / REFERENCE_W;
  
  const offsetXOrig = safeCrop.x * refToZoomedScale;
  const offsetYOrig = safeCrop.y * refToZoomedScale;
